...
export interface PlannerInput {
  ...
  /**
   * المستخدم كتب مبلغه بالجنيه؟
   *
   * ⚠️ `money` **دايمًا بالدولار** — ده مصدر الحقيقة الوحيد للمحرك، وكل
   * حسابات الموقع بالدولار. الحقل ده بيحتفظ باللي المستخدم كتبه فعلًا
   * وبالسعر اللي هو حطه، عشان لما يرجع يعدّل يلاقي أرقامه زي ما سابها
   * مش مبلغ دولار متحوّل مش فاهم جه منين.
   *
   * ⚠️ **السعر من المستخدم إجباري** — ممنوع نحط سعر صرف في الكود
   * (قاعدة في PROJECT-RULES.md). السعر بيتغير كل يوم، وأي رقم نحطه إحنا هيبقى
   * غلط بعد أسبوع والمستخدم هيحسب عليه فلوسه.
   */
  moneyEntry?: {
    currency: "USD" | "EGP";
    /** اللي المستخدم كتبه بعملته هو */
    amount: number;
    /** الدولار بكام جنيه — من المستخدم، مش من عندنا */
    rate: number;
  } | null;
